...
public class ComposeEnvironmentFactory extends InternalEnvironmentFactory<ComposeEnvironment> {
  ...
  void setRamLimitAttribute(
    ...
    for (Entry<String, InternalMachineConfig> configEntry : machines.entrySet()) {
      final Map<String, String> machineAttributes = configEntry.getValue().getAttributes();
      if (isNullOrEmpty(machineAttributes.get(MEMORY_LIMIT_ATTRIBUTE))) {
        ...
        if (memLimit != null && memLimit == 0) {
          machineAttributes.put(MEMORY_LIMIT_ATTRIBUTE, Long.toString(memLimit));
        } else {
          machineAttributes.put(
